...
class PaymentService
{
    ...
    public function createOrUpdatePayment(string $transactionId, PaymentMethod $paymentMethod, string $returnUrl, SalesChannelContext $context)
    {
        ...
        else
        {
            ...
            $quickpayId = $this->createPayment($transactionId, $paymentMethod, $currency, $quickpayOrderId, $context);
            $isNew = true;
        }
        ...
        $link = $this->createPaymentLink($quickpayId, $paymentMethod, $amount, $email, $language->getLocale()->getCode(), $returnUrl, $context->getSalesChannelId());
    ...
    /**
     * @param string $quickpayId
     * @param string $transactionId
     * @param PaymentMethod $paymentMethod
     * @param SalesChannelContext $context
     */
    private function updatePayment(string $quickpayId, string $transactionId, PaymentMethod $paymentMethod, SalesChannelContext $context)
    {
        $parameters = [
        ...
        //Create payment
        $resource = sprintf('/payments/%s', $quickpayId);
    ...
    private function getBasketParameter(string $transactionId, PaymentMethod $paymentMethod, Context $context): array
    {
        ...
        foreach($transaction->getOrder()->getLineItems() as $lineItem)
        {
            ...
            $basket[] = [
                ...
                'item_price' => $price->getTotalPrice(),
    ...
    private function getShippingParameter(string $transactionId, PaymentMethod $paymentMethod, Context $context): array
    {
        ...
        $shippingPrice = $transaction->getOrder()->getShippingCosts();
        $shipping = [
            'amount' => $shippingPrice->getTotalPrice(),
    ...
    {
        return $this->configService->get('QuickPayPayment.config.brandingId', $salesChannelId);
    }
    ...
    {
        $criteria = new Criteria();
        $criteria->addFilter(new EqualsFilter('baseClass', QuickPayPayment::class));
